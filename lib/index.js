...
async function patchCra(index_html, App, route, url, rootSelector) {
  ...
  // Injecting rendered markup into "#root" element
  const dom = new JSDOM(index_html);
  const $root = dom.window.document.querySelector(rootSelector)
  $root.innerHTML = markup;

  // <script> tag with __CONTEXT__
  function insertAfter(newNode, referenceNode) {
    referenceNode.parentNode.insertBefore(newNode, referenceNode.nextSibling);
  }
  const script = dom.window.document.createElement("script");
  script.innerHTML = `window.__CONTEXT__ = ${serialize(context)}`
  insertAfter(script, $root);

  return dom.serialize()
}

function ssrr(App, routes, options = {}) {
  defaults(options, {
  ...
  const {buildPath, target} = options
  ...
  app.use(async function (req, res, next) {
    try {
      ...
      async function patch(html) {
        const newHtml = await patchCra(html, App, route, req.url, options.rootSelector);
        ...
        proxy.once('proxyRes', function (proxyRes, req, res) {
          ...
          proxyRes.on('end', async function () {
            try {
              ...
              //
              if (req.headers["accept-encoding"] && req.headers["accept-encoding"].includes('gzip') && finalBody.length > 0) {
